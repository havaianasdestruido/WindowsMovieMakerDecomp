...
// PE validation
// ============================================================
static void TestPEValidation(HMODULE h) {
    printf("\n========================================\n");
    printf("  PE Validation\n");
    printf("========================================\n");
    ...
    printf("  Machine:       0x%04X (x86=%s)\n",
        nt->FileHeader.Machine,
        nt->FileHeader.Machine == 0x14C ? "yes" : "no");
...
// Module info
// ============================================================
static void PrintModuleInfo(HMODULE h) {
    printf("\n========================================\n");
    ...
    printf("[*] Full path: %ls\n", modPath);
    HANDLE hFile = CreateFileW(modPath, GENERIC_READ, FILE_SHARE_READ, NULL,
        OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile != INVALID_HANDLE_VALUE) {
        LARGE_INTEGER fileSize;
        if (GetFileSizeEx(hFile, &fileSize))
            printf("[*] File size: %lld bytes (0x%llX)\n", fileSize.QuadPart, fileSize.QuadPart);
        CloseHandle(hFile);
    }
}
...
int main(int argc, char** argv) {
    ...
    if (hViewer) {
        TestPEValidation(hViewer);
        TestComExports(hViewer);
        TestLaunchExports(hViewer);
        PrintModuleInfo(hViewer);
